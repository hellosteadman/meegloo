...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'Post.area'
        db.add_column('streams_post', 'area', self.gf('django.db.models.fields.CharField')(default='', max_length=50), keep_default=False)
        
        if not db.dry_run:
            from meegloo.streams.helpers import reverse_geocode
            for post in orm.Post.objects.exclude(
                latitude__isnull = True,
                longitude__isnull = True
            ):
                area = reverse_geocode(post.latitude, post.longitude)
                if area:
                    orm.Post.objects.filter(
                        pk = post.pk
                    ).update(
                        area = area
                    )
    ...
    def backwards(self, orm):
        ...
        # Deleting field 'Post.area'
        db.delete_column('streams_post', 'area')
